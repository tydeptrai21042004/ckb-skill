...
SkillPass should be positioned as **portable service-right infrastructure for CKB applications and AI agents**, not as another paywalled demo API.
...
Fiber's April 2026 agent-integration design identifies a missing **Fiber Gateway / reverse-proxy layer**, service discovery, cryptographic credentials, delegation, and agent-oriented API access as useful building blocks. Source: https://github.com/nervosnetwork/fiber/issues/1255
...
3. **Owner-signed agent delegation** — the current owner can grant a short-lived `invoke` credential to a separate CKB address while ownership remains in the owner's Capability Cell.
...
9. **Second useful built-in service** — `research-insights-v1` adds manuscript structure/readability/citation/equation signals and demonstrates that the platform is no longer tied to one handler.
10. **Agent SDK** — dependency-light discovery and signed invocation helpers expose payment requirements cleanly to agent callers.
...
> **SkillPass Gateway: protect any existing read/idempotent JSON API with a transferable CKB Capability, optional Fiber/x402 payment, and owner-signed temporary agent delegation.**
...
- owner delegates one service to an agent wallet for <= 24 hours;
- agent invokes without receiving the owner's private key or Capability Cell;
...
The first delegation primitive scopes service/action/outpoint/expiry, but does not maintain per-grant call counters or payment budgets. Add durable grant usage accounting only when a concrete agent workflow needs it. Doing this correctly requires shared state across replicas.
...
1. Fiber payment adapter for the Agent SDK so it can satisfy a 402 requirement and retry with a fresh intent-bound challenge.
...
The funding story is stronger when SkillPass is evaluated as an infrastructure primitive rather than as a single demo application. v1.2 therefore adds three concrete pieces that map to real agent-payment production gaps:

1. **Budgeted delegation** — the owner can authorize an agent for a bounded number of calls and/or a bounded Fiber spend amount without transferring the durable CKB right.
...
3. **Agent protocol + payment adapter** — agents can discover the gateway, obtain fresh intent signatures, present delegated credentials, surface/pay x402 requirements through a pluggable adapter, then retry with a fresh challenge.
...
A natural next milestone is a write-safe job gateway with a client-signed operation ID, durable outbox, upstream idempotency acknowledgement, reconciliation UI, and end-to-end agent-to-agent paid service demo. That milestone should be treated separately from the current read/query gateway because payment settlement and irreversible upstream side effects require stronger recovery guarantees.
