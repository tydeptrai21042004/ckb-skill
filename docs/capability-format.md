...
Capability v1 uses a fixed **106-byte** payload. All offsets are byte offsets.
...
Expiry policy: a pass is active only when `now_unix_seconds < expiry`. At `now == expiry`, it is expired. The Type Script permits ownership-preserving or transferable transitions after expiry; the protected service denies use after expiry. This avoids non-deterministic wall-clock logic inside the v1 Type Script.
